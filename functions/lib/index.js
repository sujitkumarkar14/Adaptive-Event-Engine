...
admin.initializeApp();
const mapsPlatformKey = (0, params_1.defineSecret)("MAPS_PLATFORM_KEY");
const emergencyBroadcastKey = (0, params_1.defineSecret)("EMERGENCY_BROADCAST_KEY");
const vertexIngestKey = (0, params_1.defineSecret)("VERTEX_INGEST_KEY");
/**
 ...
 */
const USE_MOCK_DATA = process.env.FUNCTIONS_EMULATOR === "true"
    ? process.env.USE_MOCK_DATA !== "false"
    : process.env.USE_MOCK_DATA === "true";
/** Constant-time UTF-8 string comparison (length mismatch returns false without timingSafeEqual). */
function safeEqualUtf8(provided, expected) {
    const a = Buffer.from(provided !== null && provided !== void 0 ? provided : "", "utf8");
 ...
 */
function logAuditJson(payload) {
    const line = JSON.stringify(Object.assign(Object.assign({}, payload), { timestamp: new Date().toISOString() }));
 ...
 */
exports.reserveEntrySlot = functions.https.onCall({ secrets: [mapsPlatformKey] }, async (request) => {
    if (!request.auth) {
        logAuditJson({
            severity: "NOTICE",
    ...
    const { slotId, gateId } = request.data;
    if (!slotId || !gateId) {
        logAuditJson({
            severity: "NOTICE",
    ...
    const spanner = new spanner_1.Spanner();
    ...
    try {
        ...
        if ((e === null || e === void 0 ? void 0 : e.message) === "CAPACITY_EXHAUSTED") {
            logAuditJson({
                severity: "NOTICE",
            ...
            throw new functions.https.HttpsError("resource-exhausted", "Capacity exhausted for this slot.");
        }
        logAuditJson({
            severity: "ERROR",
        ...
        throw new functions.https.HttpsError("internal", "Reservation could not be completed. Try again later.");
    }
});
/**
 ...
 */
exports.calculateOptimalPath = functions.https.onCall(async (request) => {
    var _a;
    if (!request.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Authentication required.");
    }
    const { originLat, originLng, destinationGate, stepFreeRequired } = ((_a = request.data) !== null && _a !== void 0 ? _a : {});
    if (USE_MOCK_DATA) {
 ...
 */
exports.vertexAggregator = (0, https_1.onRequest)({ secrets: [vertexIngestKey], region: "us-central1", cors: false }, async (req, res) => {
    const deploymentMode = process.env.FUNCTIONS_EMULATOR === "true" ? "EMULATOR_MODE" : "PRODUCTION_MODE";
    ...
    const provided = headerKey !== null && headerKey !== void 0 ? headerKey : bodyKey;
    ...
    const emulatorMockAuthorized = emulatorMode && safeEqualUtf8(provided, "MOCK_VERTEX_INGEST_KEY");
    const authorized = prodAuthorized || emulatorMockAuthorized;
    if (!authorized) {
        functions.logger.warn(`[AUDIT] vertexAggregator deployment=${deploymentMode} result=unauthorized`);
        res.status(401).send("Unauthorized");
        return;
    }
    if (emulatorMode && emulatorMockAuthorized) {
        functions.logger.warn(`[AUDIT] vertexAggregator deployment=EMULATOR_MODE auth=MOCK_VERTEX_INGEST_KEY note=not_for_production`);
    }
 ...
 */
exports.broadcastEmergency = (0, https_1.onRequest)({ secrets: [emergencyBroadcastKey], region: "us-central1", cors: false }, async (req, res) => {
    const deploymentMode = process.env.FUNCTIONS_EMULATOR === "true" ? "EMULATOR_MODE" : "PRODUCTION_MODE";
    ...
    const authorized = prodAuthorized || emulatorMockAuthorized;
    if (!authorized) {
        functions.logger.warn(`[AUDIT] broadcastEmergency deployment=${deploymentMode} result=unauthorized`);
        res.status(401).send("Unauthorized");
        return;
    }
    if (emulatorMode && emulatorMockAuthorized) {
        functions.logger.warn(`[AUDIT] broadcastEmergency deployment=EMULATOR_MODE auth=MOCK_SECRET_KEY note=not_for_production`);
    }
    functions.logger.info(`[AUDIT] broadcastEmergency deployment=${deploymentMode} result=authorized dispatch=firestore`);
